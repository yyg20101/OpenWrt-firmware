...
| Firmware CI run `27060485731` | Successful on `main`, commit `a57f1cb703bb2acaebec17f7b32c44dbd06ea122`, target `x86_64_all`, `release=false`; jobs `x86_64_LEDE` and `x86_64_immortalWrt` both completed with conclusion `success`. | 2026-06-06 |
| Firmware CI run `27060485731` artifact inventory | Produced all expected x86 artifacts: config-audit, compile-log, firmware, and smoke artifacts for both `x86_64_LEDE` and `x86_64_immortalWrt`. Firmware artifacts uploaded successfully (`1872121413` bytes for LEDE and `1841212360` bytes for ImmortalWrt), and smoke artifacts uploaded successfully (`305902762` bytes for LEDE and `229345234` bytes for ImmortalWrt). | 2026-06-06 |
| Firmware CI run `27060485731` config and compile proof | Downloaded config-audit and compile-log artifacts to `/private/tmp/openwrt-artifacts-27060485731-small`; both config audits have `missing-luci-apps.txt` size `0`, `luci-app-alist` requested/effective `n`, `autosamba` requested/effective `n`, LuCI Chinese/theme/uHTTPd/rpcd effective `y`, and PassWall plus shared third-party apps effective `y`. Compile logs show PassWall, GecoOS AC, mini-diskmanager, partexp, wolplus, and Samba4 compiled for both profiles; LEDE additionally compiled OpenVPN and mwan3helper while ImmortalWrt did not request them. | 2026-06-06 |
...
8. x86 firmware generation is confirmed on branch run `27031596559`, main run `27041924279`, config cleanup run `27050273854`, and Alist/plugin-guard removal run `27060485731`; both `x86_64_LEDE` and `x86_64_immortalWrt` succeeded in each final proof run.
...
| Third-party plugin overlays | Verified selected third-party repositories expose package Makefiles for GecoOS AC, mini-diskmanager, partexp, wolplus, LEDE-only mwan3helper, and the `kenzok8/small` package set. `luci-app-alist` has been intentionally removed. | Seed LEDE builds from `kenzok8/small@master`, overlay selected shared apps for supported source trees, keep mwan3helper only for LEDE, and audit requested apps after defconfig. | 已证明 |
...
| Alist and fixed plugin guard removal | Commit `a57f1cb703bb2acaebec17f7b32c44dbd06ea122` removes the `luci-app-alist` request and overlay, deletes the fixed plugin overlay validator, and keeps the generic requested-vs-effective LuCI audit. `rg` found no active `luci-app-alist` or `validate-plugin-overlay` references under `scripts`, `.github`, or `devices`. |
| Final x86 proof after Alist/plugin guard removal | `Firmware CI` run `27060485731` completed successfully on `main` for `target=x86_64_all`; both x86 jobs completed `Compile Firmware`, `Upload Firmware Artifact`, `Smoke X86 Artifact / 烟测 X86 产物`, and `Upload Smoke Report / 上传烟测报告` with conclusion `success`. |
| Final artifact inventory after Alist/plugin guard removal | Run `27060485731` produced all eight expected artifacts: config-audit, compile-log, firmware, and smoke for both x86 profiles. Artifact metadata confirms firmware upload sizes of `1872121413` bytes for LEDE and `1841212360` bytes for ImmortalWrt, plus smoke artifact sizes of `305902762` bytes for LEDE and `229345234` bytes for ImmortalWrt. |
| Final config-audit proof after Alist/plugin guard removal | Downloaded config-audit and compile-log artifacts to `/private/tmp/openwrt-artifacts-27060485731-small`. Both profiles have `missing-luci-apps.txt` size `0`; `luci-app-passwall`, `luci-app-gecoosac`, `luci-app-mini-diskmanager`, `luci-app-partexp`, `luci-app-wolplus`, and `luci-app-samba4` are requested/effective `y`; `luci-app-alist` and `autosamba` are requested/effective `n`; LuCI Chinese, bootstrap theme, uHTTPd, uHTTPd ubus, and rpcd LuCI are effective `y`. LEDE alone requests/effectively enables `luci-app-openvpn` and `luci-app-mwan3helper`; ImmortalWrt does not request/effectively enable them. |
