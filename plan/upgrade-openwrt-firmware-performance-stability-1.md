...
| Firmware CI run `27060485731` config and compile proof | Downloaded config-audit and compile-log artifacts to `/private/tmp/openwrt-artifacts-27060485731-small`; both config audits have `missing-luci-apps.txt` size `0`, `luci-app-alist` requested/effective `n`, `autosamba` requested/effective `n`, LuCI Chinese/theme/uHTTPd/rpcd effective `y`, and PassWall plus shared third-party apps effective `y`. Compile logs show PassWall, mini-diskmanager, partexp, wolplus, and Samba4 compiled for both profiles; LEDE additionally compiled OpenVPN and mwan3helper while ImmortalWrt did not request them. `luci-app-gecoosac` was later removed from the local requested set after run `28097015786` failed in `package/gecoosac` due to a missing parent `LICENSE`. | 2026-06-06 |
...
| Final config-audit proof after Alist/plugin guard removal | Downloaded config-audit and compile-log artifacts to `/private/tmp/openwrt-artifacts-27060485731-small`. Both profiles have `missing-luci-apps.txt` size `0`; `luci-app-passwall`, `luci-app-mini-diskmanager`, `luci-app-partexp`, `luci-app-wolplus`, and `luci-app-samba4` are requested/effective `y`; `luci-app-alist`, `luci-app-gecoosac`, and `autosamba` are no longer locally requested; LuCI Chinese, bootstrap theme, uHTTPd, uHTTPd ubus, and rpcd LuCI are effective `y`. LEDE alone requests/effectively enables `luci-app-openvpn` and `luci-app-mwan3helper`; ImmortalWrt does not request/effectively enable them. |
