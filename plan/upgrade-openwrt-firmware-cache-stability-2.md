...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-019A | Inspect upstream `feeds.conf.default` for `coolsnowwolf/lede`, `immortalwrt/immortalwrt`, and `VIKINGYFY/immortalwrt`; confirm all declare an official LuCI feed and current profiles do not set local `feeds_conf` overrides. | ✅ | 2026-06-06 |
...
| Upstream feeds audit | `coolsnowwolf/lede` declares `src-git luci https://github.com/coolsnowwolf/luci.git;openwrt-25.12`; `immortalwrt/immortalwrt` and `VIKINGYFY/immortalwrt` declare ImmortalWrt LuCI feeds; current `devices/profiles.yml` has no `feeds_conf` overrides. | 2026-06-06 |
