...
|----------|---------|----------|--------|------------------|
| High | Builds intentionally follow mutable external sources: upstream repos, feeds, package repos, remote build environment script, and runner images. | `.github/workflows/firmware-build.yml`, `devices/profiles.yml`, `scripts/common/Packages.sh` | Builds may fail or produce different artifacts without local changes. | Keep daily builds active and record source/package refs, profile hashes, and Release checksums for diagnosis. |
...
| Package overlay helper performs live GitHub operations. | Firmware customization intentionally follows latest package sources. | `scripts/common/Packages.sh` | Build reproducibility is limited. | Preserve resolved refs in `package-source-manifest.tsv` and verify published checksums instead of pinning versions. |
...
|------|--------------------------------|----------|--------------------|-----|
| Remote shell execution for build environment initialization. | N/A | `.github/workflows/firmware-build.yml` | Uses HTTPS and records the downloaded script checksum in build provenance. | The mutable script is executed before an independent trust decision can be made. |
...
| Cache cleanup can remove useful accelerators when retention is too aggressive. | `.github/workflows/cache-maintenance.yml` | Cleanup uses fixed thresholds, scopes caches to `refs/heads/main`, and keeps the newest cache per group. | A poorly chosen fixed threshold can still remove reusable older caches. | Run `preview` first and keep cache keys grouped by source/profile family. |
