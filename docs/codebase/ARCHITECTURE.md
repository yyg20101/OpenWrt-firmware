...
5. Scheduled builds always execute the selected matrix, even when the upstream commit and existing Release are unchanged.
...
| Profile hash provenance | `profiles.sh`, `release-maintenance.sh` | Identifies which profile config/hooks/fragments produced a stable Release. |
| Actions storage maintenance | `cache-maintenance.yml`, `actions-storage-maintenance.sh` | Runs scheduled artifact/cache/workflow-run cleanup; manual operation exposes a read-only `preview` and fixed-policy `cleanup`. |
