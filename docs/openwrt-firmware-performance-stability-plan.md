...
- `scripts/common/config/luci-zh-cn.config` 只选择 `CONFIG_LUCI_LANG_zh_Hans=y`，让官方 LuCI 规则为已安装模块带出中文翻译包；`luci-web.config` 仅给未启用上游 LuCI 默认包集合的 fork/衍生分支使用。
...
| P1-06 LuCI 官方默认审计 | `scripts/common/config/luci-zh-cn.config`、`scripts/common/config/luci-web.config`、`scripts/ci/audit-config.sh` | 不手写每个插件中文包或 LuCI runtime/lib 依赖；官方默认已包含 LuCI 的源码分支不再本地选择 `CONFIG_PACKAGE_luci=y`，未启用上游默认的 fork/衍生分支才通过 `luci-web.config` 显式选择 LuCI 元包。 | defconfig 后审计看到 `luci-base`、`luci-i18n-base-zh-cn`、uHTTPd、uHTTPd ubus、rpcd LuCI 和至少一个 LuCI 主题。 |
