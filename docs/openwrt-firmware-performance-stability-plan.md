...
| CON-006 | 优先使用官方 feeds、LuCI 和 uHTTPd 默认。 | 当前 profile 不覆盖 `feeds.conf.default`；LEDE 与官方 ImmortalWrt release branch 依赖上游默认 LuCI，未启用该默认的 fork/衍生分支才使用 `luci-web.config` 兜底；主题/uHTTPd/rpcd/LuCI runtime 由 LuCI 元包默认依赖提供并由审计确认。 |

## 3. 当前基线

当前仓库已经具备以下基础能力：

- `devices/profiles.yml` 是 profile 唯一入口，当前启用 4 个 profile。
...
- 本地配置不再选择 `CONFIG_LUCI_LANG_zh_Hans` 或硬编码 `luci-i18n-*`；官方 LEDE/ImmortalWrt 的中文相关默认项跟随上游默认包集合和 LuCI feed 规则，编译后通过产物检测确认。`luci-web.config` 仅给未启用上游 LuCI 默认包集合的 fork/衍生分支使用。
...
| P1-06 LuCI 官方默认审计 | `scripts/common/config/luci-web.config`、`scripts/ci/audit-config.sh`、`scripts/ci/build-artifacts.sh` | 不手写每个插件中文包、LuCI 语言选项或 LuCI runtime/lib 依赖；官方默认已包含 LuCI 的源码分支不再本地选择 `CONFIG_PACKAGE_luci=y`，未启用上游默认的 fork/衍生分支才通过 `luci-web.config` 显式选择 LuCI 元包。 | defconfig 后审计看到 `luci-base`、uHTTPd、uHTTPd ubus、rpcd LuCI 和至少一个 LuCI 主题；编译后产物检测官方源 `Packages.tar.gz` 包含 `luci-i18n-base-zh-cn`。 |
