...
| P1-02 cache key 分组策略复核 | `devices/profiles.yml`、`.github/workflows/firmware-build.yml` | 保持 key 由 source slug、branch、cache group、cache week 构成；检查同源同架构 profile 是否共享合理，避免跨源码污染。 | x86 两个 profile 不因不同源码混用工具链缓存；Qualcommax profile 按 source/branch 形成隔离。 |
