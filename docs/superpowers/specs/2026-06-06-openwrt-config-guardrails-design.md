...
- `scripts/common/config/base.config` currently includes `CONFIG_PACKAGE_luci=y` and shared requested LuCI applications such as PassWall, HomeProxy, ZeroTier, Docker/Dockerman, Diskman, DDNS-Go, UPnP, WireGuard, and related support packages. `luci-app-alist` and `luci-app-gecoosac` have been intentionally removed from the requested set.
